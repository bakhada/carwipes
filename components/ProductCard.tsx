...
const ProductCard: React.FC<ProductCardProps> = ({ product, onClick }) => {
  ...
  const shareProduct = (e: React.MouseEvent, platform: 'facebook' | 'twitter' | 'pinterest') => {
    e.stopPropagation(); // Don't trigger the card's onClick
    // Generate the deep link to the current website for THIS product
  ...
  return (
    <article 
      onClick={onClick}
      className={`group bg-white rounded-[2rem] overflow-hidden border border-slate-100 transition-all hover:shadow-[0_20px_50px_rgba(0,0,0,0.05)] hover:-translate-y-2 ${onClick ? 'cursor-pointer' : ''}`}
    >
      ...
      <div className="p-8 pt-2">
        ...
        <div className="flex items-center gap-3 mb-8 pb-6 border-b border-slate-50">
          <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest mr-2">Quick Share</span>
          
          <button 
            onClick={(e) => shareProduct(e, 'facebook')}
          ...
          <button 
            onClick={(e) => shareProduct(e, 'pinterest')}
          ...
          >
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.162-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.966 1.406-5.966s-.359-.72-.359-1.782c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.654 2.569-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.056-4.86-4.991-4.86-3.398 0-5.393 2.548-5.393 5.184 0 1.027.395 2.127.889 2.726a.36.36 0 0 1 .083.343c-.091.378-.293 1.189-.332 1.355-.053.218-.173.265-.4.159-1.492-.694-2.424-2.875-2.424-4.625 0-3.769 2.737-7.229 7.892-7.229 4.144 0 7.365 2.953 7.365 6.899 0 4.117-2.595 7.431-6.199 7.431-1.211 0-2.348-.63-2.738-1.373 0 0-.599 2.282-.744 2.84-.282 1.084-1.064 2.445-1.585 3.298a12.042 12.042 0 0 0 3.658.56c6.62 0 11.988-5.367 11.988-11.988 0-6.62-5.368-11.987-11.988-11.987z"/></svg>
          </button>
        </div>
